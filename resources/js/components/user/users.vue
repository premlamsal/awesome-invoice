      ...
      <div class="d-block">
        ...
        <div class="form-group">
          ...
          <select class="form-control" v-model="user.role_id">
            <option selected v-for="role in roles" :value="role.id" v-bind:key="role.id">{{ role.name }}</option>
          </select>
          <span v-if="errors.role_id" :class="['errorText']">{{ errors.type[0] }}</span>
        </div>
      </div>
            ...
            <tbody>
              <tr v-for="user in users" v-bind:key="user.id">
                ...
                <td v-if="user.email_verified_at === null">
                  <span class="bg-danger text-white p-2">Not Verified</span>
                </td>
                <td v-else>
                ...
                <td>
                  <button class="btn btn-outline-success custom_btn_table" @click="editUser(user.id)" v-if="hasPermission('edit_user')">
                    <span class="fa fa-edit custom_icon_table"></span>
                  </button>
                  <button class="btn btn-outline-danger custom_btn_table" @click="deleteUser(user.id)" v-if="hasPermission('delete_user')">
...
export default {
  data() {
    return {
      ...
      user: {}, //for form single user data
  ...
  methods: {
    ...
    editUser(id) {
      ...
      axios
        ...
        .then(response => {
          ...
          Vue.set(this.user, "role_id", response.data.user.role.id);
          Vue.set(this.user, "role_id_old", response.data.user.role.id);
    ...
    autoCompleteTable() {
      ...
      if (this.searchTableKey != "") {
        ...
        let currObj = this;
        axios
          .post("/api/users/search", { searchQuery: this.searchTableKey })
          .then(function(response) {
            currObj.isLoading = "";
